...
def measure_layer(layer, *args):
    ...
    for x in args:
        ...
        ### ops_nonlinearity
        elif type_name in ['ReLU', 'ReLU6', 'SiLU', 'Hardsigmoid', 'Sigmoid']:
            delta_ops = x.numel()
            delta_params = get_layer_param(layer)
        ...
        ### ops_nothing
        elif type_name in ['BatchNorm2d', 'Dropout2d', 'DropChannel', 'Dropout', 'FrozenBatchNorm2d', 'GroupNorm', 'LayerNorm']:
            delta_params = get_layer_param(layer)
            ...
            delta_ops = x.numel()
                           ...
                           'NaiveSyncBatchNorm', 'Upsample', 'Sequential', 'Concat']:
            pass
        ...
        elif type_name in ['Tokenizer']:
            delta_params = get_layer_param(layer)
            delta_ops = x.numel() * 2 *192

            # import pdb; pdb.set_trace()

#        elif type_name in ['SiLU']:
#            delta_ops = x.numel() * 2
#            delta_params = get_layer_param(layer)
#        
...
if __name__ == "__main__":
    ...
    model_args = {'activation': 'reglu', 'attention_dropout': 0.2, 
                'd_ffn_factor': 1.333333333333333, 'd_token': 192, 'ffn_dropout': 0.1, 
                'initialization': 'kaiming', 'n_heads': 8, 'n_layers': 3, 'prenormalization': True, 
                'residual_dropout': 0.0, 'token_bias': True, 'kv_compression': None, 
                'kv_compression_sharing': None}
    # import pdb; pdb.set_trace()
    
    model = Transformer(d_numerical=21, d_out=75, **model_args)
    ...
    image = torch.zeros(*input_size).to(device)

    _, ops, params = measure_model(model, x = image)
